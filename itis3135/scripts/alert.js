...
function intro() {
	...
	if (usersName != null) {
		document.getElementById("functionResponse").innerHTML =
			...
			" welcomes you, " +
			usersName +
			"!<br/>We're glad you are doing " +
			usersFeeling;
	}
}

/**
 * Calculates the tax on a given amount of money.           
 * @returns None           
 */
function calculateTax() {
	const TAX_RATE = 0.0525;
	var input = parseInt(
		prompt("Tax Rate is 5.25%. Enter $ amount to find tax amount: ")
	);
	if (input > 0) {
		document.getElementById("functionResponse").innerHTML =
			...
			input +
			"<br/>Tax Amount: $" +
			(TAX_RATE * input).toFixed(2) +
			"<br/>Total: $" +
			(TAX_RATE * input + input).toFixed(2);
	} else {
...
function areaPerimeter() {
	...
	if (length > 0 && height > 0) {
		document.getElementById("functionResponse").innerHTML =
			...
			height +
			"<br/><br/>Area: " +
			(length * height).toFixed(2) +
			"<br/>Perimeter: " +
			(length * 2 + height * 2).toFixed(2);
	} else {
		alert("Please try again and enter a positive number.");
	}
}
...
function milesPerGallon() {
	...
	if (miles > 0 && gallons > 0) {
		document.getElementById("functionResponse").innerHTML =
			...
			"<br/>Gallons used: " +
			gallons +
			"<br/><br/>Miles Per Gallon: " +
			(miles / gallons).toFixed(2);
	} else {
		alert("Please try again and enter a positive number.");
	}
}
...
function gpaCalculator() {
	...
	for (var i = 0; i < classes; i++) {
		...
		while (
			...
			grade = prompt(
				"Please enter grade (a,b,c,d,f) received in class " + (i + 1) + ":"
			);
		}
		grades.push(grade);
	}
